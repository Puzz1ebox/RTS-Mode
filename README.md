...
It currently does:
   - Skips the intro cinematic
   - Skips the crashsite generation
   - Puts player in an RTS view, without a character, on join/respawn
   - Prevents player from building too close to enemy spawners
   - Prevents player from attacking enemy spawners with throwables
   - Converts any outstanding recipes that are craftable by a player, but not by nil, into recipes craftable by nil.
